...
call log %logger% I Chinese text:devmode:%devmode%.func:%func%.target:%target%
::Chinese text
...
ECHOC {%c_e%}%devmode% Chinese text{%c_i%}{\n}& call log %logger% F %devmode% Chinese text& goto FATAL


:CHK-ADB
set slot_cur=
call log %logger% I Chinese text ADB Chinese text ro.boot.slot_suffix
for /f "tokens=1 delims=_ " %%a in ('adb.exe shell getprop ro.boot.slot_suffix') do set slot_cur=%%a
call log %logger% I ADB Chinese text ro.boot.slot_suffix Chinese text:%slot_cur%
...
::call log %logger% I Chinese text ADB Chinese text /proc/cmdline
::adb.exe shell < %tmpdir%\cmd.txt 1>%tmpdir%\output.txt 2>&1 || ECHOC {%c_e%}ADB Chinese text /proc/cmdline Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}&& type %tmpdir%\output.txt>>%logfile%&& call log %logger% E ADB Chinese text /proc/cmdline Chinese text&& pause>nul && ECHO. Chinese text ... && goto CHK-ADB
...
ECHOC {%c_e%}Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}& call log %logger% E Chinese text& pause>nul & ECHO. Chinese text ... & goto CHK-ADB
:CHK-ADB-1
call log %logger% I ADB Chinese text:slot_cur:%slot_cur%.slot_cur_oth:%slot_cur_oth%
::Chinese text
...
call log %logger% I Chinese text ADB Chinese text %target%(%var%)
adb.exe shell < %tmpdir%\cmd.txt 1>>%logfile% 2>&1 || ECHOC {%c_e%}ADB Chinese text %target%(%var%)Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}&& call log %logger% E ADB Chinese text %target%(%var%)Chinese text&& pause>nul && ECHO. Chinese text ... && goto SET-ADB
call log %logger% I ADB Chinese text %target%(%var%)
ECHOC {%c_we%}Chinese text %target%{%c_i%}{\n}
ENDLOCAL
goto :eof
...
fastboot.exe getvar current-slot 1>%tmpdir%\output.txt 2>&1 || ECHOC {%c_e%}Fastboot Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}&& type %tmpdir%\output.txt>>%logfile%&& call log %logger% E Fastboot Chinese text&& pause>nul && ECHO. Chinese text ... && goto CHK-FASTBOOT
type %tmpdir%\output.txt>>%logfile%
set slot_cur=
for /f "tokens=2 delims=_ " %%i in ('type %tmpdir%\output.txt ^| find "slot"') do set slot_cur=%%i
if "%slot_cur%"=="" ECHOC {%c_e%}Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}& call log %logger% E Chinese text& pause>nul & ECHO. Chinese text ... & goto CHK-FASTBOOT
if "%slot_cur%"=="a" set slot_cur_oth=b
if "%slot_cur%"=="b" set slot_cur_oth=a
::Chinese text
set slot_a_unbootable=& set slot_b_unbootable=
fastboot.exe getvar all 2>&1| find "slot-unbootable:" 1>%tmpdir%\output.txt 2>&1 || ECHOC {%c_w%}Fastboot Chinese text . Chinese text ...{%c_i%}{\n}&& type %tmpdir%\output.txt>>%logfile%&& call log %logger% W Fastboot Chinese text&& goto CHK-FASTBOOT-2
type %tmpdir%\output.txt>>%logfile%
...
if "%slot_a_unbootable%"=="" ECHOC {%c_e%}Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}& call log %logger% E Chinese text& pause>nul & ECHO. Chinese text ... & goto CHK-FASTBOOT
...
if "%slot_b_unbootable%"=="" ECHOC {%c_e%}Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}& call log %logger% E Chinese text& pause>nul & ECHO. Chinese text ... & goto CHK-FASTBOOT
:CHK-FASTBOOT-2
...
call log %logger% I Fastboot Chinese text:slot_cur:%slot_cur%.slot_cur_oth:%slot_cur_oth%.slot_cur_unbootable:%slot_cur_unbootable%.slot_cur_oth_unbootable:%slot_cur_oth_unbootable%.slot_a_unbootable:%slot_a_unbootable%.slot_b_unbootable:%slot_b_unbootable%
::Chinese text
...
if "%target%"=="cur" set target=%slot_cur%
if "%target%"=="cur_oth" set target=%slot_cur_oth%
if "%target%"=="a" (if "%slot_a_unbootable%"=="yes" ECHOC {%c_w%}Chinese text: Chinese text %target% Chinese text, Chinese text . {%c_i%}Chinese text %target%...{%c_i%}{\n}& call log %logger% W Chinese text %target% Chinese text)
if "%target%"=="b" (if "%slot_b_unbootable%"=="yes" ECHOC {%c_w%}Chinese text: Chinese text %target% Chinese text, Chinese text . {%c_i%}Chinese text %target%...{%c_i%}{\n}& call log %logger% W Chinese text %target% Chinese text)
call log %logger% I Chinese text Fastboot Chinese text %target%
fastboot.exe set_active %target% 1>>%logfile% 2>&1 || ECHOC {%c_e%}Fastboot Chinese text %target% Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}&& call log %logger% E Fastboot Chinese text %target% Chinese text&& pause>nul && ECHO. Chinese text ... && goto SET-FASTBOOT
call log %logger% I Fastboot Chinese text %target%
ECHOC {%c_we%}Chinese text %target%{%c_i%}{\n}
ENDLOCAL
goto :eof
...
:CHK-QCEDL-3
call log %logger% I 9008 Chinese text:slot_cur:%slot_cur%.slot_cur_oth:%slot_cur_oth%
::Chinese text
if not "%func%"=="set" ENDLOCAL & set slot__cur=%slot_cur%& set slot__cur_oth=%slot_cur_oth%& goto :eof
goto SET-QCEDL
:CHK-QCEDL-FAILED
type %tmpdir%\output2.txt>>%logfile%
ECHOC {%c_e%}Chinese text . {%c_h%}Chinese text ...{%c_i%}{\n}& call log %logger% E Chinese text& pause>nul & ECHO. Chinese text ... & goto CHK-QCEDL
:SET-QCEDL
if "%target%"=="cur" set target=%slot_cur%
if "%target%"=="cur_oth" set target=%slot_cur_oth%
call log %logger% I Chinese text 9008 Chinese text %target%
::Chinese text gpt Chinese text
set num=0
:SET-QCEDL-1
...
fh_loader.exe --port=\\.\COM%chkdev__port% --memoryname=%info__qcedl__memtype% --search_path=%tmpdir% --sendxml=cmd.xml --mainoutputdir=%tmpdir% --skip_config --noprompt 1>>%logfile% 2>&1 || ECHOC {%c_e%}setbootablestoragedrive Chinese text %value% Chinese text{%c_i%}{\n}&& call log %logger% F setbootablestoragedrive Chinese text %value% Chinese text&& goto SET-QCEDL-FAILED
call log %logger% I 9008 Chinese text %target%
ECHOC {%c_we%}Chinese text %target%{%c_i%}{\n}
ENDLOCAL
goto :eof
:SET-QCEDL-FAILED
ECHOC {%c_e%}Chinese text %num% Chinese text %target% Chinese text{%c_i%}{\n}& call log %logger% F Chinese text %num% Chinese text %target% Chinese text
goto FATAL
